...
<script>
  ...
  var getProcessingDelayStatus = function(init){
	  var inputPartCode;
	  if(init){
		  inputPartCode = null;
	  } else {
	  ...
	  var sendData = $("#processingdelaystatus").serialize();
	  $.ajax({
			type : "POST",
			data : sendData,
			url : contextPath+"/monitoring/processingdelaystatus",
			cache : false,
			dataType : "JSON",
			success : function(result) {
				makeTableList(result.data);
	        },
  ...
  var makeTableList = function(datas){
	  $("#dataListTbody").empty();
	  ...
	  if (datas.length != 0){
		  for (eachData in datas){
			  ...
			  if (isInit){
				  ...
				  let makeRowSpanTd = document.createElement("td");
				  makeRowSpanTd.rowSpan = datas.length;
				  makeRowSpanTd.id="tableDate";
				  makeRowSpanTd.style.borderBottom = '1px solid #E0E0E0';
				  isInit = false;
				  
				  let makeDefNameTd = document.createElement("td");
				  makeDefNameTd.innerHTML = datas[eachData].defName;
				  makeDefNameTd.style.borderBottom = '1px solid #E0E0E0';
				  let makePassedCountTd = document.createElement("td");
				  makePassedCountTd.innerHTML = datas[eachData].passedCOUNT;
				  makePassedCountTd.style.borderBottom = '1px solid #E0E0E0';
				  let makeDelayedCounTd = document.createElement("td");
				  makeDelayedCounTd.innerHTML = datas[eachData].delayedCOUNT;
				  makeDelayedCounTd.style.borderBottom = '1px solid #E0E0E0';
				  let delayedDayAvg = document.createElement("td");
				  delayedDayAvg.innerHTML = datas[eachData].delayedDayAVG;
				  delayedDayAvg.style.borderBottom = '1px solid #E0E0E0';
				  makeTr.appendChild(makeRowSpanTd);
				  makeTr.appendChild(makeDefNameTd);
				  makeTr.appendChild(makePassedCountTd);
				  makeTr.appendChild(makeDelayedCounTd);
				  makeTr.appendChild(delayedDayAvg);
			  } else {
				  makeTr = document.createElement("tr");			  
				  let makeDefNameTd = document.createElement("td");
				  makeDefNameTd.innerHTML = datas[eachData].defName;
		  ...
		  dataList.html("<td colspan='5' style='text-align: center'>데이터가 존재하지 않습니다</td>");
	  }
  ...
  $( function() {
	  ...
	  $("#searchButton").click(function (e) {
		  getProcessingDelayStatus(false);
	  })
...
<form name="processingdelaystatus" id="processingdelaystatus" method="post" action="">
